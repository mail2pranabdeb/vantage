        ...
        return args -> {
            ...
            if (!menuExists(2020L)) {
                jdbcTemplate.update(
                    ...
                    2020L, "Job Calendar", 200L, 20, "/system/job-calendar", "", "C", "0", "1", "system:job:calendar", "fa fa-calendar", "0", "admin", LocalDateTime.now(), "Job Calendar View"
                );
                addRoleMenu(1L, 2020L);
                addedCount++;
                log.info("Added menu: Job Calendar");
            }
            ...
            if (!menuExists(2021L)) {
                jdbcTemplate.update(
                    "INSERT INTO sys_menu (menu_id, menu_name, parent_id, order_num, url, target, menu_type, visible, is_refresh, perms, icon, status, create_by, create_time, remark) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    2021L, "Holiday Calendar", 200L, 21, "/system/holiday-calendar", "", "C", "0", "1", "system:job:calendar", "fa fa-calendar", "0", "admin", LocalDateTime.now(), "Holiday Calendar Management"
                );
                addRoleMenu(1L, 2021L);
                addedCount++;
                log.info("Added menu: Holiday Calendar");
            }
            ...
            if (!menuExists(2022L)) {
                jdbcTemplate.update(
                    "INSERT INTO sys_menu (menu_id, menu_name, parent_id, order_num, url, target, menu_type, visible, is_refresh, perms, icon, status, create_by, create_time, remark) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    2022L, "Live Logs", 200L, 22, "/system/job-logs", "", "C", "0", "1", "system:job:list", "fa fa-terminal", "0", "admin", LocalDateTime.now(), "Real-time Job Logs"
                );
                addRoleMenu(1L, 2022L);
                addedCount++;
                log.info("Added menu: Live Logs");
            }
            ...
            if (!menuExists(2023L)) {
                jdbcTemplate.update(
                    "INSERT INTO sys_menu (menu_id, menu_name, parent_id, order_num, url, target, menu_type, visible, is_refresh, perms, icon, status, create_by, create_time, remark) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    2023L, "Email Templates", 200L, 23, "/system/email-templates", "", "C", "0", "1", "system:job:template", "fa fa-envelope", "0", "admin", LocalDateTime.now(), "Email Template Management"
                );
                addRoleMenu(1L, 2023L);
                addedCount++;
                log.info("Added menu: Email Templates");
            }
            ...
            if (!menuExists(1034L)) {
                ...
                addedCount++;
                log.info("Added menu: Email Config");
            }
