...
public class QuartzTaskExecutor extends AbstractQuartzJob {
    ...
    private String getConfigValue(String... keys) {
        try {
            SystemMailService mailService = applicationContext.getBean(SystemMailService.class);
            return mailService.getConfigValue(keys);
        } catch (Exception e) {
            return null;
        }
    }

    private Optional<Object> findDictDataById(Long dictCode) {
        try {
            SystemDictService dictService = applicationContext.getBean(SystemDictService.class);
            return dictService.findDataById(dictCode).map(d -> (Object) d);
        } catch (Exception e) {
    ...
    private void sendReportEmail(SysJob sysJob, String templateName, String outputFormat, List<?> data) throws Exception {
            ...
            try {
                ...
                for (String dictCodeStr : dictCodes) {
                    ...
                    try {
                        ...
                        Optional<?> optDict = findDictDataById(dictCode);
        ...
        try {
            ...
            SystemMailService mailService = applicationContext.getBean(SystemMailService.class);
            JavaMailSender dynamicMailSender = mailService.getMailSender();
