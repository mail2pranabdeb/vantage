...
public class EmailTemplateRepository {
     ...
     */
    public List<EmailTemplate> findByStatus(String status) {
        CriteriaBuilder cb = entityManager.getCriteriaBuilder();
        CriteriaQuery<EmailTemplate> cq = cb.createQuery(EmailTemplate.class);
        Root<EmailTemplate> root = cq.from(EmailTemplate.class);
        if ("0".equals(status)) {
            ...
            cq.select(root).where(cb.equal(root.get("isActive"), false));
        }
        return entityManager.createQuery(cq).getResultList();
    }
     ...
     */
    public Optional<EmailTemplate> findByTemplateCode(String templateCode) {
        CriteriaBuilder cb = entityManager.getCriteriaBuilder();
        CriteriaQuery<EmailTemplate> cq = cb.createQuery(EmailTemplate.class);
        Root<EmailTemplate> root = cq.from(EmailTemplate.class);
        cq.select(root).where(cb.equal(root.get("templateCode"), templateCode));
