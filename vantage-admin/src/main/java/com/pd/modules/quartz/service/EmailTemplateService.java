...
public String processTemplate(String template, SysJob job, SysJobLog jobLog) {
    ...
    /**
     * Render template with variables (API method)
     */
    public String renderTemplate(String templateCode, Map<String, Object> variables) {
        Optional<EmailTemplate> templateOpt = templateRepository.findByTemplateCode(templateCode)
                .or(() -> templateRepository.findByName(templateCode));
        if (templateOpt.isEmpty()) {
        ...
        for (Map.Entry<String, Object> entry : variables.entrySet()) {
            body = body.replace("${" + entry.getKey() + "}", entry.getValue() != null ? entry.getValue().toString() : "");
        }
        return body;
    }

    /**
     * Send email from template (API method)
     */
    public void sendEmailFromTemplate(Long templateId, String toEmail, Map<String, Object> data) {
        ...
        try {
            ...
            for (Map.Entry<String, Object> entry : data.entrySet()) {
                subject = subject.replace("${" + entry.getKey() + "}", entry.getValue() != null ? entry.getValue().toString() : "");
                body = body.replace("${" + entry.getKey() + "}", entry.getValue() != null ? entry.getValue().toString() : "");
            }
            MimeMessage message = mailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(message, true, "UTF-8");
            ...
            mailSender.send(message);
            log.info("Email sent from template {} to {}", templateId, toEmail);
        } catch (Exception e) {
            log.error("Failed to send email from template {}", templateId, e);
        }
    }
